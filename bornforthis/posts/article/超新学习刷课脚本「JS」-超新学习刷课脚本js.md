---
title: 超新学习刷课脚本「JS」
date: 2021-03-02 12:57:41.0
updated: 2022-03-31 07:52:39.689
url: /archives/301.html
categories: 技术杂谈
tags: 
---
...
if (url == '/mycourse/studentstudy') {
...
} else if (url.match(/^\/ananas\/modules\/(pptpdf)\/index\.html$/) && setting.docs && setting.tip) {
    setTimeout(function () {
    ...
    $ && checkToNext();
} else if (url.match(/^\/(coursezt)\/\d+\.html$/)) {
    setTimeout(function () {
...
} else if (url.match(/^\/visit\/(coursesinteraction)$/)) {
...
function hookVideo() {
    ...
    var player = vjs.apply(this, arguments),
        a = '<a href="https://d0.ananas.chaoxing.com/download/' + _self.config('objectid') + '" target="_blank" rel="noopener noreferrer">',
...
function fillAnswer($li, obj, type) {
    ...
    obj.code > 0 && $input.each(function (index) {
        if (this.value == 'true') {
            data.join().match(/(^,)(正确是对√Tri)(,$)/) && this.click();
        } else if (this.value == 'false') {
            data.join().match(/(^,)(错误否错×Fwr)(,$)/) && this.click();
        } else {
