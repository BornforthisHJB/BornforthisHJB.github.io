...
此篇文章，是为了完善 Python 办公自动化的课程，有兴趣的报名一对一办公自动化的小伙伴，可以加 V：Jiabcdefh。

如果我们要操作文件、目录，可以在命令行下面输入操作系统提供的各种命令来完成。比如 dir、cp 等命令。

**如果要在 Python 程序中执行这些目录和文件的操作怎么办？**

<font color="red">其实操作系统提供的命令只是简单地调用了操作系统提供的接口函数， Python 内置的 os 模块也可以直接调用操作系统提供的接口函数。</font>
...
![在这里插入图片描述](./10.assets/de4ccb78cfc74ed48d821f15a0143387.png)
...
![在这里插入图片描述](./10.assets/6f7e3d3b826f4c8486ce0ed26e2a99cc.png)
...
![在这里插入图片描述](./10.assets/b740964fd1a74761bde12e8f1c6c24a4.png)
...
Out[9]: '\\Users\\clela'
```
![在这里插入图片描述](./10.assets/6a05f59a64564f6db3faed9d298fb3c2.png)
...
![在这里插入图片描述](./10.assets/190befe5c8a841d8b5a991fa50543008.png)
...
![目录树结构](./10.assets/24782b5f2c5f74c5971ef267e87f00ba.png)
...
但无论直接原地重命名还是“剪切粘贴”，中间路径都必须要存在，否则就会抛出 `FileNotFoundError` 异常。如果目标路径已存在，Windows 下会抛出 `FileExistsError` 异常；Linux 下，如果目标路径为空且用户权限允许，则会静默覆盖原路径，否则抛出 `OSError` 异常，
...
![在这里插入图片描述](./10.assets/fb5fa959216846b39a9d8df85fc050c8.png)
命令行显示如下：
...
![在这里插入图片描述](./10.assets/07dbe3e5e9c440d6b44b2251c76e881e.png)
...
我们可以打开可以看到，名称已经成功修改了。

![在这里插入图片描述](./10.assets/fee896915a384528a89ac2f5fb39b51a.png)
```ipython
In [6]: ls
 驱动器 C 中的卷是 OS
 卷的序列号是 0AED-8BC3
...
![在这里插入图片描述](./10.assets/e3d44785af8f46f3a3fa455a1d4b913e.png)
...
上面的代码无法使用，因为当前 `aiyc_book` 这个文件夹是不存在的。
...
![在这里插入图片描述](./10.assets/f014708dcec14ad8943bbf2501847b1c.png)
