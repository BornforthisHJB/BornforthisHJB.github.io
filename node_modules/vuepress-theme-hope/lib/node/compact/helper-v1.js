...
export const navbarConfig = (config) => {
    logger.warn('"navbarConfig" is deprecated, please import and use "navbar" from vuepress-theme-hope instead.');
    return config;
};
/**
...
export const sidebarConfig = (config) => {
    logger.warn('"sidebarConfig" is deprecated, please import and use "sidebar" from vuepress-theme-hope instead.');
    return config;
};
/**
...
const checkMarkdownOptions = (options = {}) => {
    ...
    if ("lineNumbers" in options) {
        logger.warn('"markdown.lineNumbers" is deprecated in VuePress2, please use "markdown.code.lineNumbers" instead.');
    ...
    if ("externalLinks" in options) {
        logger.error('"markdown.externalLinks" is no longer supported in VuePress2, please use "markdown.links.externalAttrs" instead.');
        delete options["externalLinks"];
    }
    // plugins
    if ("plugins" in options) {
        logger.error('"markdown.plugins" is no longer supported in VuePress2, please use "extendsMarkdown" hook instead.');
...
const checkPluginOptions = (plugins) => {
            ...
            if (typeof item === "object") {
                ...
                [
                    ...
                    if (deprecatedOption in item)
                        logger.warn(`"${deprecatedOption}" options in plugin options is deprecated in VuePress2, please use "${newOption}" instead.`);
                ...
                [
                    ...
                    if (removedOption in item)
                        logger.error(`"${removedOption}" option in plugin options is no longer supported in VuePress2, make sure you are using a VuePress2 plugin.`);
                    // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access
    ...
    if (typeof plugins === "object") {
        logger.error('VuePress2 does not support object format "plugins" anymore, you should import plugins and call them in an array.');
        return [];
    }
    return [];
};
export const checkBundlerOptions = (config) => {
    [
        ...
        if (removedOption in config)
            logger.error(`"${removedOption}" option in config file is no longer supported in VuePress2, you should set it in bundler options.`);
        // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access
...
export const config = (userConfig) => {
    ...
    [
        ...
        if (deprecatedOption in userConfig)
            logger.warn(`"${deprecatedOption}" option in config file is deprecated in VuePress2, please use "${newOption}" instead.`);
        // eslint-disable-next-line
    ...
    [
        ...
        if (removedOption in userConfig)
            logger.error(`"${removedOption}" option in config is no longer supported in VuePress2${hint ? `, ${hint}.` : "."}`);
        // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access
        delete userConfig[removedOption];
    });
    // other options
    if ("extraWatchFiles" in userConfig) {
        logger.error('"extraWatchFiles" options is removed in VuePress2, you should use "onWatched" hook.');
