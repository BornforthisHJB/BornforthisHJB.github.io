...
<details class="custom-container details"><summary>聊天记录图片</summary>
...
<p>你们细品，细细品，我不希望你们被割韭菜还不自知，这不是简单的会与不会的问题...... 一般没有实际工作经验的，只会使用 <code v-pre>time.sleep()</code>  对于小白入门来说，能想到用这个方法是值得表扬的。而当小白问老师的时候，老师回答：<code v-pre>time.sleep()</code>  ，那这个回答太不负责任也是没有任何经验就来当老师了。</p>
<h2 id="_2-你能得到什么" tabindex="-1"><a class="header-anchor" href="#_2-你能得到什么" aria-hidden="true">#</a> 2. 你能得到什么？</h2>
...
<h2 id="_3-实现" tabindex="-1"><a class="header-anchor" href="#_3-实现" aria-hidden="true">#</a> 3. 实现</h2>
<h3 id="_3-1-原理讲解" tabindex="-1"><a class="header-anchor" href="#_3-1-原理讲解" aria-hidden="true">#</a> 3.1 原理讲解</h3>
...
<h3 id="_3-2-导库" tabindex="-1"><a class="header-anchor" href="#_3-2-导库" aria-hidden="true">#</a> 3.2 导库</h3>
...
<li><code v-pre>from urllib import parse</code> ：解析 URL</li>
<li><code v-pre>from datetime import datetime</code> ：获取时间（标注时间）时间加减</li>
<li><code v-pre>import time,requests</code> ：time 实现睡眠 requests 爬虫库</li>
</ul>
<h3 id="_3-3-主体代码实现" tabindex="-1"><a class="header-anchor" href="#_3-3-主体代码实现" aria-hidden="true">#</a> 3.3 主体代码实现</h3>
<h4 id="_1-delaywait" tabindex="-1"><a class="header-anchor" href="#_1-delaywait" aria-hidden="true">#</a> 1. DelayWait</h4>
<p>这里因为我们需要自己 DIY 一个插件，所以需要创建一个类 <code v-pre>DelayWait</code> ，来使用。也方便之后的调用，不必重复编写。 我们并为这个类进行初始化，初始化如下：</p>
<div class="language-python ext-py line-numbers-mode"><pre v-pre class="language-python"><code><span class="token keyword">class</span> <span class="token class-name">DelayWait</span><span class="token punctuation">:</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><strong>上面代码看不懂？别急，我来解释！</strong> 这个其实从上面就可以看见，我们设置了延迟的时间的变量 <code v-pre>self.delay = delay</code> ， 创建了一个 <code v-pre>self.urls = dict()</code> 来存储各种 url。</p>
<ul>
<li>内置函数初始化： __init__</li>
<li>一般来说，在我们实行这个类的的时候，就会自动执行这个类的初始化函数（它是第一个执行的）</li>
<li>初始化参数（初始化属性，属性：对象的某个静态特征）</li>
<li><code v-pre>delay = 3</code> , 每次延迟 3 秒；</li>
<li><code v-pre>self.urls = dict()</code> ，存储各种 URL</li>
</ul>
<h4 id="_2-wait" tabindex="-1"><a class="header-anchor" href="#_2-wait" aria-hidden="true">#</a> 2. wait</h4>
<p>接下来我们来编写个等待函数 <code v-pre>wait</code> ，和上面一样，还是先来看看代码然后进行解析：</p>
<div class="language-python ext-py line-numbers-mode"><pre v-pre class="language-python"><code>    <span class="token keyword">def</span> <span class="token function">wait</span><span class="token punctuation">(</span>self<span class="token punctuation">,</span> url<span class="token punctuation">)</span><span class="token punctuation">:</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>我也来一行一行解析，带你读懂它：</p>
<ol>
<li><code v-pre>netloc = parse.urlparse(url).netloc</code></li>
</ol>
<ul>
<li>函数：对象的某个动态能力</li>
...
<ol start="2">
<li><code v-pre>lastOne = self.urls.get(netloc)</code></li>
</ol>
<ul>
<li>我们上一步请求的网址是什么，看有没有这个主站；</li>
<li>这里的 <code v-pre>get('', null)</code> 是字典的一个方法，使用 get 来查询字典中的数据，如果这个数据存在，则返回改键对应的值。</li>
<li>不存在则返回：预先设定的内容：null；</li>
</ul>
<ol start="3">
<li><code v-pre>if lastOne and self.delay&gt;0:</code></li>
</ol>
<ul>
<li>如果，我们上一次请求过的话，就执行这个语句</li>
</ul>
<ol start="4">
<li><code v-pre>timeWait = self.delay - (datetime.now()-lastOne).seconds</code></li>
</ol>
<ul>
...
<li>解析:
<ul>
<li>如果两次请求的时间差 <code v-pre>(datetime.now()-lastOne)</code> 非常小,表明,两次请求的时间间隔,非常小；</li>
...
<ol start="5">
<li><code v-pre>self.urls[netloc] = datetime.now()</code></li>
</ol>
<ul>
<li>为字典 urls 添加：键对值：主站:对应添加的时间</li>
</ul>
<h2 id="_4-完整代码" tabindex="-1"><a class="header-anchor" href="#_4-完整代码" aria-hidden="true">#</a> 4. 完整代码</h2>
