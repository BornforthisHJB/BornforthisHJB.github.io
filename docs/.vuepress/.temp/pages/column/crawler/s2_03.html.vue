...
<h2 id="_1-认识浏览器和服务器" tabindex="-1"><a class="header-anchor" href="#_1-认识浏览器和服务器" aria-hidden="true">#</a> 1. 认识浏览器和服务器</h2>
...
<p>前面也说过，这个问题可以讲上三天三夜，但我们没那么多时间，其中一些细节就略过，把大致流程结合爬虫讲一讲，分成三部分：「强迫症或者想认真补的学员呢，可以点击此文章阅读」<RouterLink to="/column/crawler/replenish03.html"><strong>从输入 url 到页面展现发生了什么？</strong></RouterLink></p>
<ol>
<li>浏览器发出请求</li>
<li>服务器做出响应</li>
<li>浏览器接收响应</li>
</ol>
<h2 id="_2-浏览器发出请求" tabindex="-1"><a class="header-anchor" href="#_2-浏览器发出请求" aria-hidden="true">#</a> 2. 浏览器发出请求</h2>
...
<h3 id="_2-1-网址是不是有效的" tabindex="-1"><a class="header-anchor" href="#_2-1-网址是不是有效的" aria-hidden="true">#</a> 2.1 网址是不是有效的？</h3>
<p>首先，浏览器要判断你输入的网址（URL）是否合法有效。对应 URL，同学们并不陌生吧，以 http(s) 开头的那一长串的字符，但是你知道它还可以以 <code v-pre>ftp</code> ,  <code v-pre>mailto</code> , <code v-pre>file</code> , <code v-pre>data</code> , <code v-pre>irc</code>  开头吗？下面是它最完整的语法格式：</p>
<div class="language-python ext-py line-numbers-mode"><pre v-pre class="language-python"><code>URI <span class="token operator">=</span> scheme<span class="token punctuation">:</span><span class="token punctuation">[</span><span class="token operator">//</span>authority<span class="token punctuation">]</span>path<span class="token punctuation">[</span>?query<span class="token punctuation">]</span><span class="token punctuation">[</span><span class="token comment">#fragment]</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>用图更形象的表现处理就是这样的：</p>
...
<p>Python 里面可以用 <code v-pre>urllib.parse</code> 来进行 URL 的各种操作</p>
<div class="language-python ext-py line-numbers-mode"><pre v-pre class="language-python"><code>In <span class="token punctuation">[</span><span class="token number">1</span><span class="token punctuation">]</span><span class="token punctuation">:</span> <span class="token keyword">import</span> urllib<span class="token punctuation">.</span>parse 
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>我们看到，urlparse 函数把 URL 分析成了 6 部分：</p>
<p><code v-pre>scheme://netloc/path;params?query#fragment</code></p>
<p>需要主要的是 netloc 并不等同于 URL 语法定义中的 host</p>
<h3 id="_2-2-服务器在哪里" tabindex="-1"><a class="header-anchor" href="#_2-2-服务器在哪里" aria-hidden="true">#</a> 2.2 服务器在哪里？</h3>
...
<h3 id="_2-3-浏览器向服务器发送些什么" tabindex="-1"><a class="header-anchor" href="#_2-3-浏览器向服务器发送些什么" aria-hidden="true">#</a> 2.3 浏览器向服务器发送些什么？</h3>
<p>浏览器获得了网站服务器的 IP 地址，就可以向服务器发送请求了。</p>
<p>这个请求就是遵循 http 协议的。写爬虫需要关心的就是 http 协议的 headers ，下面是访问 <a href="https://en.wikipedia.org/wiki/URL" target="_blank" rel="noopener noreferrer">en.wikipedia.org/wiki/URL<ExternalLinkIcon/></a> 时浏览器发送的请求 headers：</p>
...
<h3 id="_2-4-服务器返回了些什么" tabindex="-1"><a class="header-anchor" href="#_2-4-服务器返回了些什么" aria-hidden="true">#</a> 2.4 服务器返回了些什么？</h3>
