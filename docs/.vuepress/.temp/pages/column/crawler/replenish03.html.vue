...
<h2 id="_1-大致是如下步骤" tabindex="-1"><a class="header-anchor" href="#_1-大致是如下步骤" aria-hidden="true">#</a> 1. 大致是如下步骤</h2>
<ol>
<li>根据地址栏输入的地址向 <a href="https://zh.wikipedia.org/wiki/%E5%9F%9F%E5%90%8D%E7%B3%BB%E7%BB%9F" target="_blank" rel="noopener noreferrer">DNS（Domain Name System）<ExternalLinkIcon/></a> 查询IP</li>
...
<hr>
<h2 id="_2-dns" tabindex="-1"><a class="header-anchor" href="#_2-dns" aria-hidden="true">#</a> 2. DNS</h2>
<p>首先我们要知道什么是 DNS</p>
<blockquote>
...
<p>域名解析的过程是逐级查询的</p>
<ol>
<li><strong>浏览器缓存</strong>: 首先会向浏览器的缓存中读取上一次访问的记录，在 chrome 可以通过地址栏中输入 <code v-pre>chrome://net-internals/#dns</code> 查看缓存的当前状态</li>
<li><strong>操作系统缓存</strong>：查找存储在系统运行内存中的缓存。在mac中可以通过下面的命令清除系统中的DNS缓存。</li>
</ol>
<div class="language-python ext-py line-numbers-mode"><pre v-pre class="language-python"><code>dscacheutil <span class="token operator">-</span>flushcache
...
<li>
<p><strong>ISP DNS 缓存</strong>：互联网服务提供商（如中国电信）也会提供 DNS 服务，比如比较著名的 <code v-pre>114.114.114.114</code> ，在本地查找不到的情况下，就会向 ISP 进行查询，ISP 会在当前服务器的缓存内查找是否有记录，如果有，则返回这个IP，若没有，则会开始向根域名服务器请求查询。</p>
</li>
<li>
<p><strong>顶级 DNS 服务器/根 DNS 服务器</strong>：根域名收到请求后，会判别这个域名(<code v-pre>.com</code>)是授权给哪台服务器管理,并返回这个顶级 DNS 服务器的 IP。请求者收到这台顶级 DNS 的服务器IP后，会向该服务器发起查询，如果该服务器无法解析，该服务器就会返回下一级的DNS 服务器 IP（<a href="http://nicefilm.com" target="_blank" rel="noopener noreferrer">nicefilm.com<ExternalLinkIcon/></a>），本机继续查找，直到服务器找到(<a href="http://www.nicefilm.com" target="_blank" rel="noopener noreferrer">www.nicefilm.com<ExternalLinkIcon/></a>)的主机。</p>
</li>
</ol>
<p>我们可以通过 dig 命令查看域名解析的记录</p>
<div class="language-cmd ext-cmd line-numbers-mode"><pre v-pre class="language-cmd"><code>dig math.stackexchange.com
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>我们重点看返回的应答，会看到有四条记录，返回了该网址的四个 IP</p>
<div class="language-cmd ext-cmd line-numbers-mode"><pre v-pre class="language-cmd"><code>;; ANSWER SECTION:
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>31是 TTL 的值，表示该域名的缓存时间，即该时间内不用重新查询。A 是该 DNS 查询的记录类型，表示返回一个IPv4格式的地址。还有其他记录类型诸如 NS（返回查询的服务器地址）、AAAA（返回IPV6格式的地址）、CNAME（域名的别名）等。</p>
<hr>
<h2 id="_3-tcp-连接" tabindex="-1"><a class="header-anchor" href="#_3-tcp-连接" aria-hidden="true">#</a> 3. TCP 连接</h2>
...
<ul>
<li>第一次，本机将标识位 SYN 置为 1, <code v-pre>seq = x(Sequence number)</code> 发送给服务端。此时本机状态为 <strong>SYN-SENT</strong></li>
<li>第二次，服务器收到包之后，将状态切换为<strong>SYN-RECEIVED</strong>，并将标识位 SYN 和 ACK都置为1, <code v-pre>seq = y</code> , <code v-pre>ack = x + 1</code> , 并发送给客户端。</li>
<li>第三次，客户端收到包后，将状态切换为 <strong>ESTABLISHED</strong>，并将标识位 ACK 置为 1，<code v-pre>seq = x + 1</code> , <code v-pre>ack = y + 1</code> , 并发送给服务端。服务端收到包之后，也将状态切换为 <strong>ESTABLISHED</strong>。</li>
</ul>
...
<ul>
<li>标识位 ACK 置为 1 表示我已确认收到 seq 为 x 的包，并回复确认序号 <code v-pre>ack = x + 1</code></li>
...
<h3 id="_3-1-标识位-tcp-flag" tabindex="-1"><a class="header-anchor" href="#_3-1-标识位-tcp-flag" aria-hidden="true">#</a> 3.1 标识位（TCP FLAG）</h3>
...
<hr>
<h2 id="_4-http请求与响应" tabindex="-1"><a class="header-anchor" href="#_4-http请求与响应" aria-hidden="true">#</a> 4. HTTP请求与响应</h2>
<blockquote>
<p><a href="https://zh.wikipedia.org/wiki/%E8%B6%85%E6%96%87%E6%9C%AC%E4%BC%A0%E8%BE%93%E5%8D%8F%E8%AE%AE" target="_blank" rel="noopener noreferrer">超文本传输协议<ExternalLinkIcon/></a>（英文：HyperText Transfer Protocol，缩写：HTTP）是一种用于分布式、协作式和超媒体信息系统的应用层协议[1]。HTTP是万维网的数据通信的基础。设计HTTP最初的目的是为了提供一种发布和接收HTML页面的方法。通过HTTP或者HTTPS协议请求的资源由统一资源标识符（Uniform Resource Identifiers，URI）来标识。 --wiki</p>
</blockquote>
<p>我们用 <a href="https://www.segmentfault.com" target="_blank" rel="noopener noreferrer">https://www.segmentfault.com<ExternalLinkIcon/></a> 举例子。 在应用层，浏览器会分析这个 url，并设置好请求报文发出。请求报文中包括请求行、请求头、空行、请求主体。https 默认请求端口443， http 默认80。</p>
<ul>
...
<h3 id="_4-1-http缓存" tabindex="-1"><a class="header-anchor" href="#_4-1-http缓存" aria-hidden="true">#</a> 4.1 http缓存</h3>
<p>请求是浏览器的一个优化点，我们可以通过缓存来减少不必要的请求，进而加快页面的呈现。通过简单地设置http头部可以使用缓存的功能。一般来说有三种设置的方式</p>
<h3 id="_4-2-last-modify-响应头-if-modified-since-请求头" tabindex="-1"><a class="header-anchor" href="#_4-2-last-modify-响应头-if-modified-since-请求头" aria-hidden="true">#</a> 4.2 Last-Modify(响应头) + If-Modified-Since（请求头）</h3>
...
<h3 id="_4-3-etag-响应头-if-none-match-请求头" tabindex="-1"><a class="header-anchor" href="#_4-3-etag-响应头-if-none-match-请求头" aria-hidden="true">#</a> 4.3 ETag（响应头） + If-None-Match（请求头）</h3>
...
<h3 id="_4-4-cache-control-expires-响应头" tabindex="-1"><a class="header-anchor" href="#_4-4-cache-control-expires-响应头" aria-hidden="true">#</a> 4.4 Cache-Control/Expires(响应头)</h3>
...
<hr>
<h2 id="_5-页面呈现" tabindex="-1"><a class="header-anchor" href="#_5-页面呈现" aria-hidden="true">#</a> 5. 页面呈现</h2>
...
<hr>
<h2 id="_6-连接关闭" tabindex="-1"><a class="header-anchor" href="#_6-连接关闭" aria-hidden="true">#</a> 6. 连接关闭</h2>
<p>现在的页面为了优化请求的耗时，默认都会开启持久连接（keep-alive），那么一个TCP连接确切关闭的时机，是这个tab标签页关闭的时候。这个关闭的过程就是著名的四次挥手。关闭是一个全双工的过程，发包的顺序的不一定的。一般来说是客户端主动发起的关闭，过程如下。 假如最后一次客户端发出的数据 <code v-pre>seq = x</code> , <code v-pre>ack = y</code> ;</p>
<ol>
<li>客户端发送一个FIN置为1的包，<code v-pre>ack = y</code> ， <code v-pre>seq = x + 1</code> ，此时客户端的状态为 <strong>FIN_WAIT_1</strong></li>
<li>服务端收到包后，状态切换为 <strong>CLOSE_WAIT</strong> 发送一个ACK为1的包， <code v-pre>ack = x + 2</code>。客户端收到包之后状态切换为<strong>FNI_WAIT_2</strong></li>
<li>服务端处理完任务后，向客户端发送一个 <code v-pre>FIN</code> 包，<code v-pre>seq = y</code> ; 同时将自己的状态置为 <strong>LAST_ACK</strong></li>
<li>客户端收到包后状态切换为 <strong>TIME_WAIT</strong>，并向服务端发送 ACK 包，<code v-pre>ack = y + 1</code> ，等待 2MSL 后关闭连接。</li>
</ol>
<h3 id="_6-1-为什么客户端等待2msl" tabindex="-1"><a class="header-anchor" href="#_6-1-为什么客户端等待2msl" aria-hidden="true">#</a> 6.1 为什么客户端等待2MSL？</h3>
