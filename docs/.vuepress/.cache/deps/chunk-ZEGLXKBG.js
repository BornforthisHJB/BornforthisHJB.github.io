...
var useArticleCategory = (info) => {
  ...
  return computed(() => Ft(info.value.category).map((name) => ({
    name,
...
var useArticleTag = (info) => {
  ...
  return computed(() => Pt(info.value.tag).map((name) => ({
    name,
...
var setupTimelines = () => {
  ...
  const timelineItems = computed(() => {
    ...
    timelines.value.items.forEach(({ info, path }) => {
      var _a;
      const { year, month, day } = ((_a = K(info.date)) == null ? void 0 : _a.info) || {};
      if (year && month && day) {
