...
![](./05.assets/b7a296ab8d26664e03a076fa50d5b152-5583992.png)
...
其中的 `string.format()`，就是所谓的格式化函数；而大括号 `{}` 就是所谓的格式符，用来为后面的真实值——变量 name 预留位置。如果 `id = '123'`、`name='jason'`，那么输出便是：
...
f-string 这种方式可以更加简化表达过程。还支持大小写（f.或者F.） 
